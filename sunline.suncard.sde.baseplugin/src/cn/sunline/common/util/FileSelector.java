...
/**
 * <p>
 * This is a utility class used by selectors and DirectoryScanner. The functionality more properly belongs just to selectors, but unfortunately
 * DirectoryScanner exposed these as protected methods. Thus we have to support any subclasses of DirectoryScanner that may access these methods.
 * </p>
 * <p>
 * This is a Singleton.
 * </p>
...
public final class FileSelector {
	...
	private static final FileSelector instance = new FileSelector();
	 ...
	 * <p>
	 * This is not a general purpose test and should only be used if you can live with false positives. For example, <code>pattern=**\a</code> and
	 ...
	 * 
	 * @param patDirs
	 *            The tokenized pattern to match against. Must not be <code>null</code>.
	 * @param strDirs
	 *            The tokenized path to match. Must not be <code>null</code>.
	 * @param isCaseSensitive
	 *            Whether or not matching should be performed case sensitively.
	 * 
	 * @return whether or not a given path matches the start of a given pattern up to the first "**".
	...
	/**
	 * Returns dependency information on these two files. If src has been modified later than target, it returns true. If target doesn't exist, it
	 * likewise returns true. Otherwise, target is newer than src and is not out of date, thus the method returns false. It also returns false if the
	 * src file doesn't even exist, since how could the target then be out of date.
	 * 
	 * @param src
	 *            the original file
	 * @param target
	...
	/**
	 * "Flattens" a string by removing all whitespace (space, tab, linefeed, carriage return, and formfeed). This uses StringTokenizer and the default
	...
	public static String rtrimWildcardTokens(String input) {
		...
		int newLen = 0;
		String[] tokenizedPattern = tokenizePathAsArray(input);
		for (; newLen < tokenizedPattern.length; newLen++) {
			if (hasWildcards(tokenizedPattern[newLen])) {
				break;
			}
