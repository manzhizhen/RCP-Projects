/* 文件名：     StoredProcedureEditDialog.java
 ...
 * @since 1.0
 */
public class StoredProcedureEditDialog extends TitleAreaDialog {
	...
	private StoredProcedureModel oldSql;
	private StoredProcedureModel newSql;
	 ...
	 * @param parentShell
	 */
	public StoredProcedureEditDialog(Shell parentShell, StoredProcedureModel sql, List<StoredProcedureModel> sqlList) {
		super(parentShell);
		this.oldSql = sql;
		this.sqlList = sqlList;
		this.newSql = new StoredProcedureModel();
	...
	protected Control createDialogArea(Composite parent) {
		setTitle(I18nUtil.getMessage("STOREDPROCEDURE_EDIT"));
		setMessage(I18nUtil.getMessage("STOREDPROCEDURE_EDIT"));
	...
	private void initData() {
		...
		// 初始化sql内容编辑框
		sqlText.setText(oldSql.getStoredProceduerStr());
	...
	private void createListeners() {
		...
		txtName.addModifyListener(new ModifyListener() {
			...
			public void modifyText(ModifyEvent e) {
				if (txtName.getText().trim().isEmpty()) {
					setErrorMessage("存储过程名称不能为空！");
					logger.error("用户编写了空的存储过程名称");
					return;
				} else {
					setErrorMessage(null);
				}
				
				for (StoredProcedureModel model : sqlList) {
					if (model.getName().equalsIgnoreCase((txtName.getText().trim()))) {
						setErrorMessage("该存储过程名称已存在！");
						logger.error("用户编写了重复的存储过程名称");
						return;
					} else {
						setErrorMessage(null);
					}
		...
		sqlText.addModifyListener(new ModifyListener() {
			...
			public void modifyText(ModifyEvent e) {
				for (StoredProcedureModel model : sqlList) {
					if (model.getStoredProceduerStr().equalsIgnoreCase((sqlText.getText()))) {
						setErrorMessage("该存储过程已存在！");
						logger.error("用户编写了重复的存储过程");
						return;
					} else {
						setErrorMessage(null);
					}
	...
	protected void configureShell(Shell newShell) {
		newShell.setText(I18nUtil.getMessage("STOREDPROCEDURE_EDIT_DIALOG"));
	...
	protected void okPressed() {
		...
		newSql.setStoredProceduerStr(sqlText.getText().trim());
	 ...
	 * @return the newSql
	 */
	public StoredProcedureModel getNewSql() {
		return newSql;
	}
}
