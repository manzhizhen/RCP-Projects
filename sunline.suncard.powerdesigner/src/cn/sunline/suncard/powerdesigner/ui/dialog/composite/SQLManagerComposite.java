...
import cn.sunline.suncard.powerdesigner.ui.dialog.SQLEditDialog;
 ...
 * @since 1.0
 */
public class SQLManagerComposite extends Composite {
	...
	private List<SqlScriptModel> columnModelList = new ArrayList<SqlScriptModel>();
	 ...
	 * @param style
	 */
	public SQLManagerComposite(Composite parent, int style) {
		super(parent, style);
		createColumnItem();
	}
	...
	public void initColumnItemData() {
		...
		// ICellModifier modifier = new DataTypeCellModify(columnTableViewer,
		// this);
		// columnTableViewer.setCellModifier(modifier);

		columnTableViewer.setContentProvider(new ArrayContentProvider());
		columnTableViewer.setLabelProvider(new SQLManagerLabelProvider(columnModelList));
	...
	private void createColumnItem() {
		...
		/*
		 * pasButton = new Button(this, SWT.NONE); FormData fd_pasButton = new
		 * FormData(); fd_pasButton.width = 30; fd_pasButton.bottom = new
		 * FormAttachment(delButton, 0, SWT.BOTTOM); fd_pasButton.right = new
		 * FormAttachment(delButton, -6); pasButton.setLayoutData(fd_pasButton);
		 ...
		 * 
		 * copButton = new Button(this, SWT.NONE); FormData fd_copButton = new
		 * FormData(); fd_copButton.width = 30; fd_copButton.bottom = new
		 * FormAttachment(delButton, 0, SWT.BOTTOM); fd_copButton.right = new
		 * FormAttachment(pasButton, -6); copButton.setLayoutData(fd_copButton);
		 ...
		 * 
		 * cutButton = new Button(this, SWT.NONE); FormData fd_cutButton = new
		 * FormData(); fd_cutButton.width = 30; fd_cutButton.top = new
		 * FormAttachment(delButton, 0, SWT.TOP); fd_cutButton.right = new
		 * FormAttachment(copButton, -6); cutButton.setLayoutData(fd_cutButton);
		...
		TableViewerColumn(columnTableViewer, SWT.NONE);
		...
		descColumn.setText("SQL语句");

		/*
		 * TableViewerColumn tableViewerColumn_2 = new
		 * TableViewerColumn(columnTableViewer, SWT.NONE); TableColumn
		 * typeColumn = tableViewerColumn_2.getColumn();
		 * typeColumn.setWidth(130); typeColumn.setText("数据类型");
		 * 
		 * TableViewerColumn tableViewerColumn_3 = new
		 * TableViewerColumn(columnTableViewer, SWT.NONE); TableColumn
		 * lengthColumn = tableViewerColumn_3.getColumn();
		 * lengthColumn.setWidth(60); lengthColumn.setText("长度");
		 * 
		 * TableViewerColumn tableViewerColumn_4 = new
		 * TableViewerColumn(columnTableViewer, SWT.NONE); TableColumn
		 * precisionColumn = tableViewerColumn_4.getColumn();
		 * precisionColumn.setWidth(60); precisionColumn.setText("精度");
		 * 
		 * TableViewerColumn tableViewerColumn_5 = new
		 * TableViewerColumn(columnTableViewer, SWT.NONE); TableColumn pColumn =
		 * tableViewerColumn_5.getColumn(); pColumn.setWidth(25);
		 * pColumn.setText("P");
		 * 
		 * TableViewerColumn tableViewerColumn_6 = new
		 * TableViewerColumn(columnTableViewer, SWT.NONE); TableColumn fColumn =
		 * tableViewerColumn_6.getColumn(); fColumn.setWidth(25);
		 * fColumn.setText("F");
		 * 
		 * TableViewerColumn tableViewerColumn_7 = new
		 * TableViewerColumn(columnTableViewer, SWT.NONE); TableColumn mColumn =
		 * tableViewerColumn_7.getColumn(); mColumn.setWidth(25);
		 * mColumn.setText("M");
		 * 
		 * TableViewerColumn tableViewerColumn_8 = new
		 * TableViewerColumn(columnTableViewer, SWT.NONE); TableColumn iColumn =
		 * tableViewerColumn_8.getColumn(); iColumn.setWidth(25);
		 * iColumn.setText("I");
		 */
	...
	protected void addButtonEvent() {
		...
		SqlScriptModel sql = new SqlScriptModel();
		sql.setId(getNewId());
		sql.setName(getNewName());
		sql.setSqlStr("eidt sql_" + (columnModelList.size() + 1));
	...
	private void delButtonEvent() {
		List<SqlScriptModel> list = getTableSelection();
	...
	private void moveButtonEvent(boolean isUpMove) {
		...
		Map<Integer, SqlScriptModel> sortMap = new HashMap<Integer, SqlScriptModel>(); // 对上移顺序做个排序，先移动行号小的
		for (Object obj : list) {
			sortMap.put(new Integer(columnModelList.indexOf(obj)), (SqlScriptModel) obj);
		}
		...
		if (isUpMove) {
			for (int i = 0; i < columnModelList.size(); i++) {
				...
				columnModelList.set(i - 1, (SqlScriptModel) obj);
			}
		} else {
	...
	protected void copButtonEvent() {
		...
		clipboard.setContents(new Object[] { list.toArray(new String[] {}) },
	...
	protected void cutButtonEvent() {
		...
		List<SqlScriptModel> modelList = getTableSelection();
	...
	protected void openColumnDialog() {
		...
		SqlScriptModel oldSql = (SqlScriptModel) select.getFirstElement();
		SQLEditDialog sqlEditDialog = new SQLEditDialog(getShell(), oldSql, columnModelList);
		int returnCode = sqlEditDialog.open();
		if (returnCode == IDialogConstants.OK_ID) {
			SqlScriptModel newSql = sqlEditDialog.getNewSql();
	 ...
	 * @return
	 */
	public List<SqlScriptModel> getTableSelection() {
		IStructuredSelection select = (IStructuredSelection) columnTableViewer
				.getSelection();
		...
		List<SqlScriptModel> list = select.toList();
	 ...
	 * @param modelList
	 */
	public void moveColumnModelListTo(String[] models, TableItem item) {
		...
		SqlScriptModel insertModel = columnModelList.get(selectNum); // 要插入到该对象下面

		Map<Integer, String> sortMap = new HashMap<Integer, String>(); // 对上移顺序做个排序，先移动行号小的
		List<String> needSortModelList = new ArrayList<String>();
		...
		List<SqlScriptModel> downList = new ArrayList<SqlScriptModel>(); // 记录要插入对象一下的行
		...
		for (int i = 0, length = columnModelList.size()
			...
			String columnModel = sortMap.get(i);
			if (columnModel != null) {
//				columnModelList.add(columnModel);
			}
		}
	 ...
	 */
	public void setColumnModelList(List<SqlScriptModel> columnModelList) {
	...
	public List<SqlScriptModel> getColumnModelList() {
	...
	private String getNewId() {
		...
		if (length == 0) {
			return DmConstants.PRE_SQL_ID + 0;
		}
		
		int maxValue = 0;
		for (int i = 0 ; i < length ; i++) {
			String id = columnModelList.get(i).getId();
			int temp = Integer.parseInt(id.substring(DmConstants.PRE_SQL_ID.length(), id.length()));
			if (temp > maxValue) {
				maxValue = temp;
			}
		}
		
		return DmConstants.PRE_SQL_ID + (maxValue + 1);
	}
	...
	private String getNewName() {
		...
		if (length == 0) {
			return DmConstants.PRE_SQL_NAME + 0;
		}
		
		int maxValue = 0;
		for (int i = 0 ; i < length ; i++) {
			String name = columnModelList.get(i).getName();
			int temp = Integer.parseInt(name.substring(DmConstants.PRE_SQL_NAME.length(), name.length()));
			if (temp > maxValue) {
				maxValue = temp;
			}
		}
		
		return DmConstants.PRE_SQL_NAME + (maxValue + 1);
	}
}
