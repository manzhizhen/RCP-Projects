...
import cn.sunline.suncard.powerdesigner.ui.dialog.AboutUsDialog;
...
public class AboutHandler extends AbstractHandler{
	private Log logger = LogManager.getLogger(AboutHandler.class.getName());
	
	@Override
	public Object execute(ExecutionEvent event) throws ExecutionException {
		IWorkbenchWindow window = PlatformUI.getWorkbench().getActiveWorkbenchWindow();
		if(window == null) {
			logger.error("找不到活跃的WorkbenchWindow，AboutHandler执行失败！");
			return null;
		}
		
		AboutUsDialog dialog = new AboutUsDialog(window.getShell());
		dialog.open();
